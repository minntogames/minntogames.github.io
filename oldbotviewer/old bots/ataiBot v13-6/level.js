...
exports.level = function () {
  client.on("messageCreate", async (message) => {
    ...
    while (rank.text.xp >= rank.text.Nextxp) {
      ...
      rank.text.Nextxp = Math.floor(rank.text.Nextxp * probebairitu(rank.text.lv));
      if (Cirno.data.setting.Lvnotice === "true") {
        client.channels.cache
          .get(lvupNotice).send({ content: `${message.author}のレベルが${rank.text.lv}になったよ！`});
      } else {
        var a = message.member.displayName ?? message.member.nickname
        client.channels.cache.get(lvupNotice).send({ content: `${message.member.displayName}のレベルが${rank.text.lv}になったよ！`});
      }
    }
    Cirno.data.chara.lv = rank
    await datas.save(Cirno, message.author.id)
    
    if (message.content.startsWith("cio!ranking")) {
      ...
      const test = guild.members.cache.filter((member) => !member.user.bot)
      console.log(test.map((member) => member.user.id))

      for (const [memberId, member] of users) {
        const user = member.user;
        const userId = user.id;
        const nickname = member.displayName ?? user.username ?? user.globalName ?? member.nickname;
      ...
      const buttons = [
       new MessageButton() .setCustomId("r_home") .setStyle("SECONDARY") .setEmoji('1218807086809612298') .setDisabled(isFirstPage),
       ...
       new MessageButton() .setCustomId("r_end") .setStyle("SECONDARY") .setEmoji('1218807062017081354') .setDisabled(isLastPage),
     ]
  ...
  client.on('interactionCreate', async (interaction) => {
    if (!['r_back', 'r_next', 'r_home', 'r_end'].includes(interaction.customId)) {
      return;
    }
    ...
    let i, type
    if (ranking[interaction.message.id]) {
      i = ranking[interaction.message.id].page || 1 
      type = ranking[interaction.message.id].type || "T"
      ...
      ranking[interaction.message.id] = {
        page: i,
        type
      }
    } else {
    ...
    if (ranking[interaction.message.id]) {
      if (interaction.customId === 'r_home') {
        i = 1
      } else if (interaction.customId === 'r_end') {
